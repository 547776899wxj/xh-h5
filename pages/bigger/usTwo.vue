...
<template>
	<view class="content" @longpress="open" @click="open">
		<image class="bg" src="../../static/usTwo.png"></image>
		<view class="title">{{ queueTitle }}</view>
		<view class="data">
			<view class="left">
				<view class="data-title seeing-title">正在呼叫</view>
				<view class="data-title wait-title">第一诊室候诊</view>
				<view class="data-title wait-title">第二诊室候诊</view>
				<view class="data-title">已过号码</view>
			</view>
			<view class="right">
				...
				<view class="data-title wait-title" style="font-size: 40px;">
					<text>{{data.watingOne}}</text>
				</view>
				<view class="data-title wait-title" style="font-size: 40px;">
					<text>{{data.watingTwo}}</text>
				</view>
				<view class="data-title" style="display: flex; align-items: center;height: 116px;">
		...
		<popupSet ref="popupSet" @confirm="confirm" @close="close" :showTitle="true" :dataInit="dataPopup" :showPlaySound="true"></popupSet>
	</view>
</template>
			...
			this.$request({
				...
				success: datas => {
					...
					try {
						...
						let watingTwo = datas.data.waitingListTwo.map(item => {
							return item.queueNo + (item.name ? this.$util.hideName(item.name) : '');
						}).join('，');
